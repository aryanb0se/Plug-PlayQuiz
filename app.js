...
function validateCassette(data) {
  ...
  data.forEach((question, index) => {
    ...
    if (!question || typeof question.question !== 'string' || !question.question.trim()) throw new Error(`${label} needs a question.`);
    if (!Array.isArray(question.options) || question.options.length < 3 || question.options.length > 9 || question.options.some(option => typeof option !== 'string')) throw new Error(`${label} must have 3–9 text options.`);
    if (!Array.isArray(question.correct_answers) || !Number.isInteger(question.choose_x_options) || question.correct_answers.length !== question.choose_x_options || question.choose_x_options < 1 || question.choose_x_options > question.options.length) throw new Error(`${label} has mismatched correct_answers and choose_x_options.`);
    if (question.correct_answers.some(answer => !question.options.includes(answer))) throw new Error(`${label} has a correct answer that is not an option.`);
  });
  return shuffle(data).map(question => ({ ...question, options: shuffle(question.options), selected: [], submitted: false, isCorrect: false }));
}
...
function summaryView() {
  ...
  return `<div class="shell"><header class="topbar"><div class="brand"><span class="brand-mark">C</span> CASSETTE EXAM</div><div class="top-stat mono">SESSION COMPLETE</div></header><main class="summary-wrap"><p class="eyebrow mono">03 / PLAYBACK COMPLETE</p><div class="summary-head"><div><h2>Here is the<br>full playback.</h2><p class="intro">A question-by-question record of what you selected and what the cassette expected.</p></div><div class="big-score">${state.score}<small> / ${total}<br>${percentage}%</small></div></div><div class="review-list">${state.questions.map((question, index) => reviewView(question, index)).join('')}</div><div class="summary-foot"><button class="primary" id="restart-button" type="button">Load another cassette →</button></div></main></div>`;
}
...
function bindExam() {
  ...
  document.querySelector('#submit-button')?.addEventListener('click', () => { question.submitted = true; question.isCorrect = question.selected.length === question.correct_answers.length && question.correct_answers.every(answer => question.selected.includes(answer)); if (question.isCorrect) state.score++; render(); bindExam(); });
  document.querySelector('#previous-button')?.addEventListener('click', () => { if (state.current > 0) { state.current--; render(); bindExam(); } });
  document.querySelector('#next-button')?.addEventListener('click', () => { state.current++; render(); bindExam(); });
  document.querySelector('#finish-button')?.addEventListener('click', () => { state.mode = 'summary'; render(); });
  document.querySelector('#reset-button')?.addEventListener('click', () => { if (confirm('Reset this exam and discard all progress?')) reset(); });
  document.querySelector('#end-button')?.addEventListener('click', () => { state.mode = 'summary'; render(); });
}
function bindSummary() { document.querySelector('#restart-button')?.addEventListener('click', reset); }
render();
